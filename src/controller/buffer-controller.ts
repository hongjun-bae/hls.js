...
export default class BufferController extends Logger implements ComponentAPI {
  ...
  private initTracks() {
    const tracks = {};
    // Rebuilding the SourceBuffers is what an append rejection cycle does between attempts,
    // so only the per-load cycle state belongs here. Clearing the per-fragment count as well
    // would restart the budget every cycle and it would never bind.
